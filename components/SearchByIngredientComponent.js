...
const SearchByIngridientComponent = () => {
  const [searchText, setSearchText] = useState('');
  const [cocktailData, setCocktailData] = useState([]);

  const fetchCocktailData = async () => {
    try {
      const apiUrl = `http://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${searchText}`;
...
export default SearchByIngridientComponent;
